function [theta_est, theta_err, LLF, ctable_mod, ctable_fit, sivals, psyc_mate, ce] = mADC_model_fit_psyc_editMOL(ctable, cvals, d_limit)
...
%EDIT MOL: cvals are the stimulus strengths for the two modalities separately
%svals is created by averaging, mvals is created to create response probabilities for both sides based on different stimulus strengths
global svals mvals
svals = mean(cvals,1);
mvals = cvals;
%%% start the fitting
...
dmat = nan(M, K);
for i = 1:M
    dmat(i,:) = dmax(i) * (mvals(i,:).^n(i))./(mvals(i,:).^n(i) + s50(i)^n(i));
end
