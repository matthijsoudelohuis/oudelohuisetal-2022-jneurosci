...
% params.clipmodrate          = 2;              %remove neurons that are 
params.normBaseline         = 0;                %Whether firing rate is normalized to baseline (100%)
...
% idx                                 = sessionData.Photostimpower>=2;
...
response = nanmean(snakemat(:,params.xtime>0.1e6 & params.xtime<0.9e6),2);

% figure; 
figure; set(gcf,'units','normalized','Position',[0.3 0.5 0.22 0.3],'color','w'); hold all;

scatter(baseline,response,45,'r','filled')
plot([0.0001 100],[0.0001 100],'k','LineWidth',2)
xlim([0.01 200])
ylim([0.01 200])
set(gca,'Xscale','log','Yscale','log')
set(gca,'XTick',[0.1 1 10 100],'XTickLabel',{'0.1' '1' '10' '100'},'YTick',[0.1 1 10 100],'YTickLabel',{'0.1' '1' '10' '100'},'FontSize',15)
xlabel('Baseline (sp s-1)')
ylabel('Opto (sp s-1)')

[tempBF,p] = bf.ttest2(baseline,response);
[tempD] = computeCohen_d(baseline,response,'paired');

fprintf('%d V1 neurons, PPC inhibition: Cohen''s d: %2.2f, BF:%2.2f\n',length(spikeData.session_ID),tempD,tempBF)
