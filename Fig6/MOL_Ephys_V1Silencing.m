...
% [Data] = MOL_GetData('E:','CHDET',{'OptoOnly'},{'2028' '2029' '2030' '2031'},[],{'sessionData' 'trialData' 'spikeData'});
[Data] = MOL_GetData('E:','CHDET',{'OptoOnly'},[],[],{'sessionData' 'trialData' 'spikeData'});
sessionData     = Data.sessionData;
trialData       = Data.trialData;
spikeData       = Data.spikeData;

spikeData.ChannelY(spikeData.ChannelY==0) = rand(sum(spikeData.ChannelY==0),1)*400+600;

%% Filter sessions:
idx                                 = sessionData.Photostimpower>=2 & strcmp(sessionData.PhotostimArea,'V1') & strcmp(sessionData.OpsinArea,'V1');
...
for iNeuron = 1:nNeurons
    if modulationratio(iNeuron)<1
        plot(meanwf(:,iNeuron),'b');
    elseif modulationratio(iNeuron)>1
       plot(meanwf(:,iNeuron),'r');
    end
end
